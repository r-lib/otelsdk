...
`tracer_provider_http$new()` returns an
...
- `url`: OTLP URL to send telemetry data to. Value is set from
  ...
  - `OTEL_EXPORTER_OTLP_ENDPOINT` environment variable + `/v1/traces`,
    or
...
- `content_type`: data format used, JSON or binary. Possible values:
  ...
  - the `OTEL_EXPORTER_OTLP_TRACES_PROTOCOL` environment variable, or
...
- `json_bytes_mapping`: encoding used for trace id and span id. Possible
  ...
  - the `OTEL_R_EXPORTER_OTLP_TRACES_JSON_BYTES_MAPPING` environment
    variable, or
...
- `use_json_name`: whether to use json name of protobuf field to set the
  ...
  - the `OTEL_R_EXPORTER_OTLP_TRACES_USE_JSON_NAME` environment
    variable, or
...
- `console_debug`: whether to print debug messages to the console. Value
  ...
  - the `OTEL_R_EXPORTER_OTLP_TRACES_CONSOLE_DEBUG` environment
    variable, or
...
- `timeout`: HTTP timeout in milliseconds. Value is set from
  ...
  - the `OTEL_EXPORTER_OTLP_TRACES_TIMEOUT}` environment variable, or
...
- `http_headers`: additional HTTP headers to send, e.g. `Authorization`
  ...
  - the `OTEL_EXPORTER_OTLP_TRACES_HEADERS` environment variable, or
...
- `ssl_insecure_skip_verify`: whether to disable SSL. Value is set from
  ...
  - the `OTEL_R_EXPORTER_OTLP_TRACES_SSL_INSECURE_SKIP_VERIFY`
...
- `ssl_ca_cert_path`: CA certificate, path to a file. Empty string uses
  ...
  - the `OTEL_EXPORTER_OTLP_TRACES_CERTIFICATE` environment variable, or
...
- `ssl_ca_cert_string`: CA certificate, as a string. Empty string uses
  ...
  - the `OTEL_EXPORTER_OTLP_TRACES_CERTIFICATE_STRING` environment
    variable, or
...
- `ssl_client_key_path`: SSL client key, path to a file. Empty string
  ...
  - the `OTEL_EXPORTER_OTLP_TRACES_CLIENT_KEY` environment variable, or
...
- `ssl_client_key_string`: SSL client key as a string. Empty string uses
  ...
  - the `OTEL_EXPORTER_OTLP_TRACES_CLIENT_KEY_STRING` environment
    variable, or
...
- `ssl_client_cert_path`: SSL client certificate, path to a file. Empty
  ...
  - the `OTEL_EXPORTER_OTLP_TRACES_CLIENT_CERTIFICATE` environment
    variable, or
...
- `ssl_min_tls`: minimum TLS version. Empty string uses the system
  ...
  - the `OTEL_R_EXPORTER_OTLP_TRACES_SSL_MIN_TLS` environment variable,
    or
...
- `ssl_max_tls`: maximum TLS version. Empty string uses the system
  ...
  - the `OTEL_R_EXPORTER_OTLP_TRACES_SSL_MAX_TLS` environment variable,
    or
...
- `ssl_cipher`: TLS cipher. Empty string uses the system default. Value
  ...
  - the `OTEL_R_EXPORTER_OTLP_TRACES_SSL_CIPHER` environment variable,
    or
...
- `ssl_cipher_suite`: TLS cipher suite. Empty string uses the system
  ...
  - the `OTEL_R_EXPORTER_OTLP_TRACES_SSL_CIPHER_SUITE` environment
    variable, or
...
- `compression`: compression to use. Possible values are `"none"`,
  ...
  - the `OTEL_EXPORTER_OTLP_TRACES_COMPRESSION` environment variable, or
...
- `retry_policy_max_attempts`: the maximum number of call attempts,
  ...
  - the `OTEL_R_EXPORTER_OTLP_TRACES_RETRY_POLICY_MAX_ATTEMPTS`
    environment variable, or
...
- `retry_policy_initial_backoff`: the maximum initial back-off delay
  ...
  - the `OTEL_R_EXPORTER_OTLP_TRACES_RETRY_POLICY_INITIAL_BACKOFF`
    environment variable, or
...
- `retry_policy_max_backoff`: the maximum backoff places an upper limit
  ...
  - the `OTEL_R_EXPORTER_OTLP_TRACES_RETRY_POLICY_MAX_BACKOFF`
    environment variable, or
...
- `retry_policy_backoff_multiplier`: the backoff will be multiplied by
  ...
  - the `OTEL_R_EXPORTER_OTLP_TRACES_RETRY_POLICY_BACKOFF_MULTIPLIER`
    environment variable, or
