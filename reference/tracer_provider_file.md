...
It writes spans to a JSONL file, each span is a line in the file, a
valid JSON value. The line separator is `\n`. The preferred file
extension is `jsonl`.
...
`tracer_provider_file$new()` returns an
...
- `flush_interval`: Interval to force flush output. A time interval
  ...
  - `OTEL_EXPORTER_OTLP_TRACES_FILE_FLUSH_INTERVAL` environment
    variable, or
...
- `flush_count`: Force flush output after every `flush_count` records.
  ...
  - `OTEL_EXPORTER_OTLP_TRACES_FILE_FLUSH_COUNT` environment variable,
    or
...
- `rotate_size`: How many rotated output files to keep. Value is set
  ...
  - `OTEL_EXPORTER_OTLP_TRACES_FILE_ROTATE_SIZE` environment variable,
    or
