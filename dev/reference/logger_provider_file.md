...
It writes logs to a JSONL file, each log is a line in the file, a valid
JSON value. The line separator is `\n`. The preferred file extension is
`jsonl`.
...
`logger_provider_file$new()` returns an
...
- `flush_interval`: Interval to force flush output. A time interval
  ...
  - `OTEL_EXPORTER_OTLP_LOGS_FILE_FLUSH_INTERVAL` environment variable,
    or
